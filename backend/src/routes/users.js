...
router.delete('/:id', protect, masterOnly, async (req, res) => {
  ...
  try {
    const result = await db.query(
      "DELETE FROM users WHERE id = $1 AND role != 'master'",
      [targetUserId]
    );
...
router.put('/:id/role', protect, masterOnly, async (req, res) => {
  ...
  try {
    const result = await db.query(
      "UPDATE users SET role = $1 WHERE id = $2 AND role != 'master'",
      [role, targetUserId]
    );
    if (result.rowCount === 0) {
      return res
...
router.get('/:id/permissions', protect, adminOnly, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT template_id FROM user_template_permissions WHERE user_id = $1',
      [req.params.id]
    );
...
router.put('/:id/permissions', protect, adminOnly, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      'DELETE FROM user_template_permissions WHERE user_id = $1',
      [userId]
    );
    ...
    if (templateIds.length > 0) {
      const insertPromises = templateIds.map((templateId) => {
        const query =
          'INSERT INTO user_template_permissions (user_id, template_id) VALUES ($1, $2)';
