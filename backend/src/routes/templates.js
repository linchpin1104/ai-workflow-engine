...
router.get('/', protect, async (req, res) => {
  ...
  if (role === 'master' || role === 'admin') {
    ...
    query =
      'SELECT t.* FROM templates t JOIN user_template_permissions utp ON t.id = utp.template_id WHERE utp.user_id = $1 ORDER BY t.name';
    params.push(userId);
  }
...
router.delete('/:id', protect, adminOnly, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      'DELETE FROM user_template_permissions WHERE template_id = $1',
      [req.params.id]
    );
    ...
    res.json({
      message: 'Template and associated permissions deleted successfully',
    });
  } catch (err) {
