...
router.post('/error', protect, async (req, res) => {
  ...
  const sql =
    'INSERT INTO error_logs (user_id, username, action_type, workflow_id, step_index, error_message, context) VALUES ($1, $2, $3, $4, $5, $6, $7)';
  const params = [
    userId,
    username,
    actionType,
...
router.get('/errors', protect, masterOnly, async (req, res) => {
  ...
  try {
    ...
    const dataSql =
      'SELECT * FROM error_logs ORDER BY timestamp DESC LIMIT $1 OFFSET $2';
...
router.get('/llm', protect, masterOnly, async (req, res) => {
  ...
  try {
    ...
    const dataSql =
      'SELECT id, timestamp, username, template_name, step_index, provider, model_id, is_success FROM llm_logs ORDER BY timestamp DESC LIMIT $1 OFFSET $2';
...
router.get('/llm/:id', protect, masterOnly, async (req, res) => {
  ...
  try {
    ...
    logger.error('Failed to fetch or parse LLM log detail from DB', {
      error: e.message,
      logId: req.params.id,
    });
